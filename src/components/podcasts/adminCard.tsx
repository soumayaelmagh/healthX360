...
function AdminPodcastCard(props: podcastType) {
  const [deletionPanel, setPanel] = useState<boolean>(false);

  const deletePodcast = async (youtubeId: string) => {
    try {
      const response = await fetch(`/api/videos/${youtubeId}`, {
        method: "DELETE",
      });
        ...
        <div className="w-fit flex flex-col items-center gap-3 md:gap-6 bg-secondary rounded-lg px-5 py-4 md:p-10 ">
          ...
          <div className="w-full flex items-center justify-between gap-3">
            ...
            <button
              ...
              onClick={async () => {
                await deletePodcast(props.youtubeId);
...
export default AdminPodcastCard;
